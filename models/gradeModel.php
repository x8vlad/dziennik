...
/*
work:
    SELECT subjects.name, grades.sub_id, ROUND(AVG(grades.grade),2) AS avg_grade
	    FROM `grades` LEFT JOIN subjects ON subjects.id=grades.sub_id 
		    WHERE user_id=1 GROUP BY grades.sub_id;

not work: 
    SELECT subjects.name, grades.sub_id, ROUND(AVG(grade),2) AS avg_grade
        FROM `grades` LEFT JOIN subjects ON subjects.id=grades.sub_id 
...
class gradeModel{
    ...
    public function getAVGGrades($userID){
        $queryAVG = "SELECT subjects.name, grades.sub_id, ROUND(AVG(grades.grade),2) AS avg_grade
	    FROM `grades` LEFT JOIN subjects ON subjects.id=grades.sub_id 
		    WHERE user_id=:user_id GROUP BY grades.sub_id;";
            ...
            $totalQuery = $stmt->fetchAll(PDO::FETCH_ASSOC);
            return $totalQuery;
    }
