...
<body>
    ...
    if (isset($_POST['btn'])) {
        ...
        function IsvalidPwd($password)
        {
            ...
            if($lengthOk && $hasDigit && $hasUpper && $hasSpecial){
                return true;
            }else{return false;}
        }
        $pwd = $_POST['pwd'];
        if (IsvalidPwd($pwd)) {
            echo "You welcome ur pwd is correct";
        } else {
            echo "Inccrect pwd try again";
        }
        ...
        // if(isNotCorrect()){echo "all ok" . $_POST['login'];}else{echo "incorrect login";}
